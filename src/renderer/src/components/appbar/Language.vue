<script setup>

</script>
<template>
    <el-popover placement="bottom" trigger="hover" popper-class="appbar-popover">
        <template #reference>
            <div class="svg-icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
                    <path
                        d="M16 4C9.382813 4 4 9.382813 4 16C4 22.617188 9.382813 28 16 28C22.617188 28 28 22.617188 28 16C28 9.382813 22.617188 4 16 4 Z M 16 6C17.96875 6 19.796875 6.585938 21.34375 7.5625L20.53125 7.65625L20.71875 9.65625L19.65625 9.1875L18.78125 9.9375L18.9375 12L21.09375 11.3125L23.78125 12.1875L23.09375 13.4375L21.46875 12.4375L19.71875 12.6875L18 13.96875L17.03125 16.96875L18.96875 18.5625C18.96875 18.5625 20.957031 18.21875 21.0625 18.21875C21.167969 18.21875 21.90625 20.03125 21.90625 20.03125L20.3125 25.03125C19.011719 25.652344 17.542969 26 16 26C15.683594 26 15.371094 25.964844 15.0625 25.9375L13.96875 24.03125L15.03125 20.03125L11 17L7.28125 17L6.3125 15.03125L9 12.90625L13 11L12.40625 8.34375L14.125 7.96875L14.9375 9.09375L17.9375 8.53125L17.40625 6.21875L15.1875 6.0625C15.453125 6.042969 15.726563 6 16 6 Z M 14.875 6.0625L13.3125 6.71875L12.5625 6.59375C13.300781 6.324219 14.066406 6.152344 14.875 6.0625 Z M 6.0625 16.78125L7.03125 17.90625L7.03125 19.96875L8.90625 22.03125L10.0625 22.03125L12.90625 25.53125C9.132813 24.308594 6.378906 20.890625 6.0625 16.78125Z"
                        fill="currentColor" />
                </svg>
            </div>
        </template>
        <template #default>
            <div class="mt-1 mb-1">
                <div v-for="i in ['中文', 'English']" class="select-none lang-item">
                    {{ i }}
                </div>
            </div>
        </template>
    </el-popover>
</template>
<style scoped>
.appbar-popover {
    padding: 0 !important;
}

.lang-item {
    transition: all 0.3s ease-in-out;
    padding-left: 0.5rem;
    min-height: 2rem;
    line-height: 2rem;
}

.lang-item:hover {
    background-color: #E5E9F0;
}

.svg-icon {
    height: 2rem;
    width: 2rem;
    transition: all 0.1s ease-in-out;
    color: #5b5b5b;
}

.svg-icon:hover {
    color: #0046f8;
}
</style>